...
const ExpandPlasmidRow = ({samples, isEditing = false, onSamplesChange}) => {
    ...
    const handleVolumeBlur = (index, e) => {
        ...
        // Auto-add .0 to whole numbers for clarity (same as InputPlasmidRow)
    ...
    return (
        <div>
            <div className="grid grid-cols-3 gap-4 text-xs font-semibold text-gray-600 px-2 pb-1">
                <div>Volume</div>
                <div>Created</div>
                <div>Modified</div>
            </div>
            {samples.map((sample, index) => (
                <div key={index} className={`grid grid-cols-3 gap-4 text-xs text-gray-600 px-2 py-1 rounded hover:bg-gray-100 transition-colors ml-px`}>
